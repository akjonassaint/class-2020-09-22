...
sep_new <- 
  read_excel("new_data/class_enrollment_summary_by_term_9-22-2020.xlsx", skip = 3) %>% 
  clean_names() %>% 
  filter(! is.na(course_title)) %>% 
  select(-grad, -non_degree, -x_reg, -vus, -employee, 
         -withdraw, -total, -instructor_full_name, -course_section_code) %>%
...
# How we can use the data to solve the problem
# Recognize we don't have all the answers
```
...
```{r}
# Recognize the limitations of our models
# Use humility 
```
